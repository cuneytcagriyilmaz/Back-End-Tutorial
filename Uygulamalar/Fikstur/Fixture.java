...
public class Fixture {
    public List<List<Match>> getFixtures(List<String> teams) {
        ...
        for (int roundNumber = 0; roundNumber < rounds.size(); roundNumber++) {
            if (roundNumber % 2 == 1) {
                ...
                rounds.get(roundNumber).set(0, new Match(match.getGuestTeam(), match.getHomeTeam())); // flip it
            }
        }
        ...
        for(List<Match> round: rounds){
            ...
            for(Match match : round){
                reverseRound.add(new Match(match.getGuestTeam(), match.getHomeTeam())); // flip it
            }
    ...
    public void print(List<List<Match>> all) {
        ...
        for(List<Match> round: all){
            ...
            count++;
            for(Match match:round){
                System.out.println(match.getHomeTeam()+"-"+match.getGuestTeam());
            }
